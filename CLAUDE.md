...
Applying a preset also records it as `PlayerbotManagerDB.lastApplied` (a `{name, members}` snapshot). On **`PLAYER_LEVEL_UP`**, `PlayerbotManager_OnLevelUp` (registered in `BuildUI`'s event frame, branched by `event` name) re-initialises the bots: `.warstormbot bot init=epic` (SAY) → `WhisperSpecs(BuildSpecQueue(lastApplied.members))` → `autogear` (PARTY). It no-ops when not in a party; if no preset was ever applied it still does init + autogear (no specs to re-whisper). The behaviour is gated by `PlayerbotManagerDB.autoLevelUp` (**default on**, seeded in `Init`), toggled for now via the slash command `/wbm levelup [on|off]` — it has no UI checkbox yet (placement TBD). `/wbm` with no args toggles the panel.
